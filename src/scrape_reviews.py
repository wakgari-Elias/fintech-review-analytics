from google_play_scraper import Sort, reviews  # Library to scrape Google Play reviews
import pandas as pd  # For data manipulation and storage
from langdetect import detect, DetectorFactory  # For detecting language of reviews
...
def scrape_bank_reviews(app_id, bank_name, target_count=450, batch_size=200):
    ...
    while len(all_reviews) < target_count and attempts < 10:
        ...
        for r in result:
            review_text = r.get("content", "").strip()  # Get review text and remove extra spaces
            if review_text and is_english(review_text):  # Only keep English reviews
                all_reviews.append({
                    "review": review_text,
...
def main():
    ...
    for bank, info in apps.items():
        df = scrape_bank_reviews(info["app_id"], bank, target_count=info["target_count"])
        df_list.append(df)
